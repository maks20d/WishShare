...
# Check if key is missing or insecure
if not settings.jwt_secret_key or settings.jwt_secret_key in _insecure_keys or len(settings.jwt_secret_key) < 32:
    ...
    if env.lower() == "local":
        ...
        # Production: generate ephemeral key but warn (don't block startup)
        settings.jwt_secret_key = secrets.token_urlsafe(64)
        _dev_logger.warning("JWT_SECRET_KEY not set; using ephemeral key for this session (not secure for multi-instance)")
