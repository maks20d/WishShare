...
async def wishlist_ws(
    ...
    FIX: Creating a separate DB session for the entire WebSocket lifecycle,
    instead of using DbSessionDep which can close before WebSocket completes (race condition).
    """
    # Create a separate session for the entire WebSocket lifecycle
