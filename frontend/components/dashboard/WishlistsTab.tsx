...
import { encodePathParam } from "../../lib/routeParams";
...
export default function WishlistsTab({ onEdit, onCreateClick }: Props) {
  ...
  const copyPublicLink = async (slug: string) => {
    const url = `${window.location.origin}/wishlist/${encodePathParam(slug)}`;
        ...
        {wishlists.map((w) => (
          <article key={w.id} className="surface-panel p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <Link href={`/wishlist/${encodePathParam(w.slug)}`} prefetch={false} className="btn-primary text-sm">
                Открыть
              </Link>
