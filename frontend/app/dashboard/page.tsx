...
export default function DashboardPage() {
  const { user, logout, sessionChecking } = useAuthStore();
  ...
  if (sessionChecking && !user) {
    return (
      <main className="min-h-screen px-4 py-10 grid-mesh">
        <div className="max-w-lg mx-auto surface-panel-strong p-8 text-center space-y-3">
          <h1 className="text-2xl font-semibold">Проверяем сессию</h1>
          <p className="text-sm text-[var(--text-secondary)]">Подождите, загружаем ваш кабинет...</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen px-3 sm:px-4 py-6 md:py-10 grid-mesh">
      <div className="max-w-6xl mx-auto space-y-4 md:space-y-6">
        <header className="surface-panel-strong p-4 sm:p-6 md:p-8 grid gap-4 md:gap-6 xl:grid-cols-[1fr_auto] xl:items-center">
          <div className="space-y-2">
            <p className="text-xs uppercase tracking-[0.3em] text-[var(--text-secondary)]">Управление</p>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-semibold">Ваши вишлисты</h1>
            <p className="text-sm text-[var(--text-secondary)] max-w-2xl">
              Создавайте списки, отслеживайте прогресс и делитесь ссылками с друзьями.
            </p>
          </div>
          <div className="grid grid-cols-2 sm:flex gap-2 sm:gap-3 xl:justify-end">
            <div className="surface-panel px-3 sm:px-4 py-2.5 sm:py-3 min-w-[120px] border border-[var(--line-strong)]">
              <p className="text-xs text-[var(--text-secondary)]">Вишлистов</p>
              <p className="text-xl sm:text-2xl font-semibold">{stats.totalWishlists}</p>
            </div>
            <div className="surface-panel px-3 sm:px-4 py-2.5 sm:py-3 min-w-[120px] border border-[var(--line-strong)]">
              <p className="text-xs text-[var(--text-secondary)]">Подарков</p>
              <p className="text-xl sm:text-2xl font-semibold">{stats.totalGifts}</p>
            </div>
            <button
              onClick={async () => {
                await logout();
              ...
              className="btn-ghost col-span-2 sm:col-auto"
            >
              Выйти
            </button>
          </div>
